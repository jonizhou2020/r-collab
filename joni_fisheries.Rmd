...
```
From `salmon_us_annual` create a line graph with year on the x-axis, and tot_value on the y-axis.

```{r}
ggplot(data = salmon_us_annual, aes(x = year, y = tot_value, group = subgroup)) +
  geom_line(aes(color = subgroup)) +
  theme_bw() +
  labs(x = "Year", y = "US Commercial salmon value (USD)", title = "my awesome graph")
